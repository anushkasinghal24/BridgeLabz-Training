...
public class ContactAppController {
    ...
    @PostMapping("/create")
    public ResponseEntity<ResponseDTO> create(@Valid @RequestBody RequestDTO req) {
    ...
    public ResponseEntity<ResponseDTO> update(@PathVariable int id, @Valid @RequestBody RequestDTO req) {
        ContactApp contact = ContactApp.builder()
                .name(req.getName())
                .email(req.getEmail())
                .mobNo(req.getMobNo())
